...
| `/tiktok` | Dashboard de canal | conexão, período, visão financeira, status dos pedidos, backlog e estados reais de ausência de dados | mesma anatomia interna da Amazon via `FinancialSummaryPanel`; faturamento capturado e composição pendente ficam explícitos antes da cascata custo/resultado/margem; faixas compartilham padding/gap, rótulos de 13px e valores de 16px; o estado compartilhado de conexão usa títulos de 18px e texto operacional de 12–15px | validada no navegador | Crystal Fancy manteve contexto financeiro em até duas linhas, com padding compacto, faixa complementar de 57px e ausência de overflow; donut parcial, disclosure dos sete componentes, resultado/margem neutros e nenhuma lacuna convertida em zero |
