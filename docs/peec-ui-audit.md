...
| `/amazon` | Dashboard de canal | período, pendências, resumo financeiro, conciliação, vendas, estoque, rentabilidade e produtos | anatomia transversal `período → contexto → briefing → estado → métricas → tendência → ranking → módulos secundários`; gráfico com métrica ativa, crosshair, tooltip e variação semântica | validada no navegador | mesma geometria de filtro/cabeçalho dos demais canais, cinco KPIs, ranking logo após análise temporal e ausência de overflow confirmados no Chrome; falta mobile real |
| `/mercado-livre` | Dashboard de canal | faturamento, cancelamentos, lucro/cobertura, pedidos, estoque, custos, série diária, saldo, pedidos recentes e top produtos | mesma anatomia transversal dos dashboards de canal; atualização migrou para o filtro global e sincronização fica junto do briefing | validada no navegador | filtro e cabeçalho nas mesmas coordenadas do TikTok; métricas, gráfico e ranking na mesma ordem, dados reais e ausência de overflow confirmados no Chrome |
| `/shopee` | Dashboard de canal | conexão, sincronização, faturamento/pedidos e estados reais de ausência de dados | `IntegrationDashboardFrame` também nos estados de erro, loading, atenção, reconexão, ausência de credencial, conexão e primeira sincronização | validada no navegador | ausência de conexão, ação OAuth e shell compartilhado validados sem executar escrita; estado preenchido segue como lacuna real |
| `/tiktok` | Dashboard de canal | conexão, período, visão financeira, backlog e estados reais de ausência de dados | mesma anatomia transversal; cinco KPIs prioritários → disclosure dos sete componentes preservados → gráfico → ranking → backlog/configuração/cobertura | validada no navegador | loja Crystal Fancy, disclosure nativo, 12 componentes financeiros preservados, ordem decisória, gráfico e ausência de overflow validados; interações mutáveis não acionadas |
...
| `/shopee/abc` | Análise ABC | classes A/B/C por receita e estado de conexão | módulo compartilhado por canal + estado inicial orientado | implementada | ausência de loja conectada preservada; estado preenchido continua uma lacuna externa real |
...
| `/shopee/produtos` | Listagem editável | custos por produto, cobertura e estados de sincronização | módulo compartilhado por canal + estado inicial orientado | implementada | ausência de conexão validada; estado preenchido depende de loja real |
...
| `/shopee/estoque` | Listagem operacional | estoque, giro, ruptura e estado de cobertura | módulo compartilhado por canal + estado inicial orientado | implementada | ausência de conexão validada; estado preenchido depende de loja real |
...
| `/shopee/catalogo` | Listagem de anúncios | catálogo, filtros e estado de sincronização | módulo compartilhado por canal + estado inicial orientado | implementada | ausência de conexão validada; estado preenchido depende de loja real |
...
| `/calculadora`, `/amazon/calculadora` | Ferramenta de decisão | consulta por ASIN, preço, custo, imposto e comparação FBA/FBM/DBA | entrada compacta → configuração fiscal contextual → três cenários explicáveis somente após cálculo | implementada | composição inicial, foco, campos, imposto desconhecido e erro real de consulta externa validados no Chrome; resultado preenchido aguarda a Price/Fees API responder nesta conta |
