...
| `/landing` | Entrada pública | proposta de valor, canais, benefícios, demonstração financeira e entradas para autenticação | hero com grid estrutural visível → prateleira branca curva com tabs flutuantes → wash pastel NEXO restrito à vitrine → moldura externa e janela interna da demonstração → componentes e estados reais do sistema | validada no navegador | amostra local identificada, troca automática/manual 15/30 dias, três abas, métricas animadas, gráfico explorável e donut validados no Chrome; cursor guiado abre o seletor de canais, passa pela visão consolidada e segue para período/métrica; “Todos os canais” exibe os quatro marketplaces, totais e leitura rápida; grid, ombros arredondados, bezel e faixa cromática comparados lado a lado com a geometria da Dub em desktop |
