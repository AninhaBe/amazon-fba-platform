...
| `/login` | Autenticação | login, criação de conta com confirmação de senha no cliente e servidor, autocomplete assistido de e-mail, visibilidade independente das senhas, validação, erro, retorno e acesso à política de privacidade | grade estrutural no canvas → narrativa “Sua operação, conectada” → três promessas operacionais → marketplaces convergindo no símbolo NEXO → formulário delineado com tabs locais; movimento permanece decorativo e respeita redução de movimento | validada no navegador | texto final, grade, trilhas, sugestões Gmail/Hotmail/Outlook/Yahoo/iCloud, visibilidade das senhas, bloqueio quando divergem e liberação quando coincidem validados no Chrome sem enviar credenciais; console limpo |
