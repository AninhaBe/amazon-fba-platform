...
| `/amazon` | Dashboard de canal | período, pendências, resumo financeiro, conciliação, vendas, estoque, rentabilidade e produtos | anatomia transversal `período → contexto → briefing → cinco métricas financeiras → seis indicadores operacionais → gráfico + composição → ranking → módulos secundários`; gráfico com métrica ativa, crosshair, tooltip e variação semântica | validada no navegador | faixa operacional ficou em seis colunas sem quebra no desktop; composição, gráfico e viewport mobile sem overflow confirmados no Chrome |
| `/mercado-livre` | Dashboard de canal | faturamento, cancelamentos, lucro/cobertura, pedidos, estoque, custos, série diária, saldo, pedidos recentes e top produtos | mesma anatomia da Amazon: cinco métricas financeiras, seis indicadores operacionais, gráfico explorável e painel de resultado com donut somente quando a cobertura permite; pendências de custo, cancelamento e estoque permanecem acionáveis | validada no navegador | dados reais, ordem, disclosure dos custos, neutralidade do lucro incompleto e mobile sem overflow confirmados; imposto ausente continuou desconhecido em vez de zero |
| `/shopee` | Dashboard de canal | conexão, sincronização, faturamento/pedidos, escrow, custos e estados reais de ausência de dados | estado preenchido implementado com a mesma anatomia da Amazon/ML: cinco métricas, seis indicadores, gráfico, donut condicionado à cobertura e cascata de resultado; estados de erro/loading/conexão continuam no `IntegrationDashboardFrame` | implementada | ausência de conexão e ação OAuth validadas sem escrita; estado preenchido não pode ser validado visualmente enquanto não houver loja real conectada |
| `/tiktok` | Dashboard de canal | conexão, período, visão financeira, status dos pedidos, backlog e estados reais de ausência de dados | cinco métricas financeiras → disclosure dos componentes → seis indicadores operacionais/status → gráfico + composição financeira → ranking → backlog/configuração/cobertura; distribuição de status saiu do painel lateral e entrou na faixa comparável | validada no navegador | Crystal Fancy desktop/mobile, custos expansíveis com todos os componentes, resultado neutro enquanto o ledger está parcial, gráfico e ausência de overflow confirmados; nenhuma mutação acionada |
