...
| `/pesquisa`, `/amazon/pesquisa` | consulta, FBA, sort, carregar mais, monitoramento e vazios de busca | coberto com consulta real, resultado, hierarquia de métricas, ordenação e ações preservadas; monitoramento não foi acionado |
...
|---|---|---|---|---|---|
| Todas as rotas autenticadas | Shell | troca de canal/conta, navegação agrupada, busca, integrações, briefing, onboarding e logout | sidebar única 240→48 com reflow persistido, topbar de 48px, canvas fluido, alvo de nav de 28px e tour contextual em três passos com spotlight, tooltip e retomada por “Como funciona” | validada no navegador | geometria, sequência Próximo, conclusão, `Esc`, persistência versionada e reabertura manual validadas no Chrome; falta repetir a captura em mobile real porque o override de viewport da extensão não alterou a janela externa |
| `/` | Dashboard multicanal | receita, lucro parcial, margem, pedidos, cobertura, série diária e status por canal | faixa consolidada contínua → comparação tabular prioritária por canal com barras proporcionais e ação contextual → leitura rápida calculada com maior faturamento, maior ritmo e queda → gráfico diário explorável por canal; total do gráfico em 18px/tabular e período secundário; cores identificam canal e vermelho/verde preservam a semântica financeira | validada no navegador | a leitura rápida usa somente faturamento e séries reais: exclui canal desconectado/com erro, preserva zero conhecido e não inventa tendência sem semana-base; TikTok Shop, Amazon e Mercado Livre foram calculados com o snapshot autenticado; total e cabeçalho responsivo foram validados no drill-down Todos → Amazon → Todos sem corte ou overflow |
| `/amazon` | Dashboard de canal | período, pendências, resumo financeiro, conciliação, vendas, estoque, rentabilidade e produtos | anatomia transversal + painel canônico compartilhado `Resumo financeiro → composição → faturamento → custos → resultado → margem`; gráfico com métrica ativa, crosshair, tooltip e variação semântica; faixas primária e complementar usam padding de 7px, gap de 1px, rótulos de 13px e valores de 16px | validada no navegador | composição completa, lucro/margem semânticos e faixa de sete indicadores em uma única linha confirmados no Chrome; a faixa primária caiu de 99px para 79px sem perder a linha de contexto e ambas ficaram sem overflow |
| `/mercado-livre` | Dashboard de canal | faturamento, cancelamentos, lucro/cobertura, pedidos, estoque, custos, série diária, saldo, pedidos recentes e top produtos | mesma anatomia interna da Amazon via `FinancialSummaryPanel`; receita parcial abre composição com saldo pendente neutro, seguida da mesma cascata e margem; faixas primária e complementar compartilham densidade, rótulos de 13px e valores de 16px | validada no navegador | composição pendente de R$ 107,34, disclosure dos custos, resultado/margem neutros e faixas de 80px/57px validados sem overflow; imposto ausente continuou `null` |
| `/shopee` | Dashboard de canal | conexão, sincronização, faturamento/pedidos, escrow, custos e estados reais de ausência de dados | mesma anatomia interna da Amazon via `FinancialSummaryPanel`: composição, faturamento, custos expansíveis, resultado e margem; densidade das faixas é herdada do `IntegrationDashboardFrame`; o estado compartilhado de conexão usa títulos de 18px e texto operacional de 12–15px | implementada | ausência de conexão e orientação OAuth validadas sem escrita, inclusive legibilidade do componente compartilhado; o corpo preenchido segue sem validação visual porque não há loja real conectada |
| `/tiktok` | Dashboard de canal | conexão, período, visão financeira, status dos pedidos, backlog, configurações e estados reais de ausência de dados | mesma sequência dos demais canais: briefing → pendência compacta → cinco KPIs → seis indicadores operacionais → gráfico + `FinancialSummaryPanel` → drill-downs → top produtos → saldo; backlog, cobertura e imposto foram preservados sob disclosure “Operação e sincronização” | validada no navegador | Crystal Fancy validada no Chrome com oito pendências recolhidas em um único resumo, faixa operacional fixa, detalhamento por status e componentes, gráfico e composição sem conversão de lacunas em zero; console limpo e mobile segue pendente pelo limite do viewport externo |
| `/briefing`, `/amazon/briefing` | Análise acionável | alertas priorizados, evidência, impacto, estado “sob controle”, analisar novamente, adiar, dispensar, resolver e próximo passo | resumo contínuo → contagens por severidade → tabs locais → tabela de duas camadas inspirada na densidade do Chats → evidência e impacto centralizados em suas colunas → detalhe inline com ações; disclosure progressivo 12 por vez | validada no navegador | 12 linhas a 68px, truncamento, filtro, cabeçalho, centralização e expansão inline a 88px validados no Chrome sem executar mutações; mobile coberto por CSS e ainda requer captura real |
...
| `/pesquisa`, `/amazon/pesquisa` | Pesquisa e decisão | consulta, filtro FBA, ordenação, paginação, métricas competitivas e estados vazios | pergunta compacta → explicação em disclosure → filtros contextuais → cabeçalho contextual compacto → tabela com produto, métricas de 16 px e ações de 40 px → cards responsivos | validada no navegador | consulta por ASIN, resultado real, ordenação por posição, ausência de overflow, ações preservadas e console limpo validados no Chrome; monitoramento não foi acionado por alterar dados |
...
| `/landing` | Entrada pública | proposta de valor, canais, benefícios, demonstração financeira e entradas para autenticação | grid estrutural full-bleed no hero → faixa com bordas horizontais e réguas verticais → prateleira branca de curvas cúbicas com tabs flutuantes → wash pastel NEXO → moldura externa e janela interna da demonstração | validada no navegador | amostra local identificada, troca automática/manual 15/30 dias, três abas, métricas animadas, gráfico explorável e donut validados no Chrome; cursor guiado abre o seletor de canais, passa pela visão consolidada e segue para período/métrica; “Todos os canais” exibe os quatro marketplaces, totais e leitura rápida; grid fora do container, curvas sem bico, linhas estruturais, bezel e faixa cromática comparados lado a lado com a Dub em desktop |
| `/login` | Autenticação | login, criação de conta com confirmação de senha no cliente e servidor, autocomplete assistido de e-mail, visibilidade independente das senhas, validação, erro, retorno e acesso à política de privacidade | grade estrutural no canvas → narrativa “Sua operação, conectada” com entrada escalonada → três promessas operacionais → sinais de cada marketplace percorrem trilhas próprias, convergem no símbolo NEXO e acionam “Conectando os pontos” → formulário delineado com tabs locais; movimento permanece decorativo e respeita redução de movimento | validada no navegador | sequência observada em dois momentos no Chrome; texto final, grade, trilhas, sugestões Gmail/Hotmail/Outlook/Yahoo/iCloud, visibilidade das senhas, bloqueio quando divergem e liberação quando coincidem validados sem enviar credenciais; viewport 1745×785 sem scroll horizontal ou vertical e sem corte do formulário |
