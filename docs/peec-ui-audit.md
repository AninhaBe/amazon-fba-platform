...
| `/landing` | Entrada pública | proposta de valor, canais, benefícios, demonstração financeira e entradas para autenticação | hero/grid/navegação e primeira dobra inspirados na Dub + vitrine composta por `Metric`, `AnimatedNumber`, `RevenueChart`, `CompositionDonut`, ícones e estados reais do sistema NEXO | validada no navegador | amostra local identificada, troca automática/manual 15/30 dias, três abas, métricas animadas, gráfico explorável e donut validados no Chrome; cursor guiado agora clica em período e métrica, troca título/total/série e respeita interação manual; comparação desktop lado a lado com Dub e viewport móvel 390×844 sem overflow de documento concluídos |
