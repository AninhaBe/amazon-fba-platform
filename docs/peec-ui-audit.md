...
| `/configuracoes` | Configurações | preferência local de largura/recolhimento da navegação e acesso às configurações operacionais reais de canais, custos, impostos, integrações e privacidade | navegação por assunto → seções contínuas → estado atual e ação contextual por linha; sem opções fictícias ou grades decorativas | validada no navegador | desktop autenticado sem overflow; alternância expandido/compacto e restauração visual validadas no Chrome sem alterar dados operacionais |
| `/perfil` | Configurações | identidade autenticada, nome de exibição, e-mail e confirmação, datas da conta/sessão, atalhos de dados e encerramento de sessão | resumo de identidade → edição curta → fatos da conta → canais/dados → ação de sessão separada | validada no navegador | dados reais da sessão, estados verificado/indisponível, formulário e navegação validados no Chrome; nome e sessão não foram alterados durante a validação |
