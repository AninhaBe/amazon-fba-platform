...
| `/landing` | Entrada pública | proposta de valor, canais, benefícios, demonstração financeira e entradas para autenticação | estrutura editorial inspirada na Dub + vitrine composta por `Metric`, `AnimatedNumber`, `RevenueChart`, `CompositionDonut`, ícones e estados reais do sistema NEXO | implementada | demonstração pública usa somente amostra local identificada, troca automática/manual de 15/30 dias, métricas animadas, gráfico explorável e donut interativo validados no Chrome; concluir comparação integral da página e captura mobile isolada |
