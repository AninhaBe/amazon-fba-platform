...
/**
 * Estima a COMISSÃO pela tabela publicada da Amazon (`source = 'tabela'`).
 *
 * ⚠️ **É o único caminho que cobre o ASIN QUE NUNCA VENDEU.** A tarifa observada
 * copia o que a Amazon já cobrou daquele produto — se ele nunca vendeu, não há o
 * que copiar. A tabela responde por CATEGORIA, então responde antes da primeira
 * venda. Medido em 01/09/2026: dos 74 ASINs vendidos da conexão, 54 tinham
 * comissão observada e 20 não.
 *
 * ORDEM DE PREFERÊNCIA, inalterada: **observada > tabela > api**. Esta função só
 * escreve onde não há tarifa real nem estimativa vigente, e a consulta abaixo
 * garante isso — a observada, que roda antes, já ocupou o que podia.
 *
 * ⚠️ **SÓ COMISSÃO. A FBA POR TABELA FICA DE FORA, e é pendência com nome:**
 * a tabela de logística da Amazon é por PESO e DIMENSÃO, mora em outra página e
 * exige capturar peso/dimensão por ASIN — trabalho próprio, não uma linha a
 * mais. Até lá a FBA observada cobre o que temos, e o ASIN que nunca vendeu fica
 * com comissão estimada e logística ausente. Ausência que a tela aponta é melhor
 * que um número de logística que ninguém publicou para aquele produto.
 */
export async function estimarPelaTabela(
  connectionId: string,
  limite = 400,
  janelaEmDias = 60,
): Promise<{
  pedidos: number; linhas: number; semCategoria: number; semPreco: number;
  recusadas: number; motivos: Record<string, number>;
}> {
  const { dbQuery } = await import("../db");
  const { currentWorkspaceId } = await import("../workspaceScope");
  ...
  const linhas = await dbQuery<{
    ...
    `SELECT i.external_order_id, i.line_no, i.external_product_id, i.qty, i.unit_price,
       ...
       JOIN workspace_channel_orders o
         ON o.workspace_id = i.workspace_id AND o.provider = i.provider
        AND o.connection_id = i.connection_id AND o.external_order_id = i.external_order_id
       LEFT JOIN workspace_marketplace_products p
         ON p.workspace_id = i.workspace_id AND p.provider = i.provider
        AND p.connection_id = i.connection_id AND p.external_product_id = i.external_product_id
      WHERE i.workspace_id = $1 AND i.provider = 'amazon' AND i.connection_id = $2
        AND o.status <> 'cancelled'
        AND o.occurred_at >= now() - ($4 || ' days')::interval
      ORDER BY o.occurred_at DESC
  ...
  const pedidosTocados = new Set<string>();
  let gravadas = 0, semCategoria = 0, semPreco = 0, recusadas = 0;
  const motivoDaRecusa = new Map<string, number>();
