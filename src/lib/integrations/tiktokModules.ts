...
/**
 * ⚠️ Itens agregados por LATERAL, NÃO por `GROUP BY`.
 *
 * A versão anterior fazia `SELECT s.*` com um `GROUP BY` de lista escrita à mão.
 * Toda coluna nova em `workspace_channel_orders` tinha que ser lembrada ali — e
 * não foi: `ordered_gross` e `ordered_gross_source` (migrations 0010 e 0011)
 * ficaram de fora e o Postgres passou a recusar a consulta inteira com 42803
 * ("must appear in the GROUP BY clause"). Resultado: `/tiktok/monitor` devolvia
 * **500 em qualquer conta com TikTok conectado**, real ou demo.
 *
 * Com LATERAL não há `GROUP BY`, então a próxima coluna não quebra nada.
 */
export async function readMonitor(connection: IntegrationConnection, params: URLSearchParams) {
  ...
  const rows = await dbQuery<any>(`WITH filtered AS (SELECT o.* FROM workspace_channel_orders o WHERE o.workspace_id=$1 AND o.provider=$2 AND o.connection_id=$3 AND o.occurred_at BETWEEN $4 AND $5 AND ($6='' OR o.external_order_id ILIKE '%'||$6||'%' OR EXISTS (SELECT 1 FROM workspace_channel_order_items si WHERE si.workspace_id=o.workspace_id AND si.provider=o.provider AND si.connection_id=o.connection_id AND si.external_order_id=o.external_order_id AND si.sku ILIKE '%'||$6||'%')) AND (cardinality($7::text[])=0 OR o.status=ANY($7::text[])) AND ($10='' OR o.external_order_id=$10) AND ($11='' OR EXISTS (SELECT 1 FROM workspace_channel_order_items fi WHERE fi.workspace_id=o.workspace_id AND fi.provider=o.provider AND fi.connection_id=o.connection_id AND fi.external_order_id=o.external_order_id AND fi.sku=$11))), counted AS (SELECT filtered.*,COUNT(*) OVER()::int total FROM filtered), selected AS (SELECT * FROM counted ORDER BY occurred_at DESC, external_order_id DESC LIMIT $8 OFFSET $9) SELECT s.*, (s.financial_settled OR COALESCE((s.raw #>> '{_sellercore,statementSettled}')::boolean,false)) statement_settled, COALESCE(it.items,'[]') items FROM selected s LEFT JOIN LATERAL (SELECT json_agg(json_build_object('productId',i.external_product_id,'sku',i.sku,'title',i.title,'qty',i.qty,'unitPrice',i.unit_price) ORDER BY i.line_no) items FROM workspace_channel_order_items i WHERE i.workspace_id=s.workspace_id AND i.provider=s.provider AND i.connection_id=s.connection_id AND i.external_order_id=s.external_order_id) it ON true ORDER BY s.occurred_at DESC,s.external_order_id DESC`, args);
  const items = rows.map((r:any) => { const evidence=r.raw?._sellercore?.financialEvidence??{}; const complete=[[r.evidence_fees,"fees"],[r.evidence_seller_shipping,"sellerShipping"],[r.evidence_ads,"ads"],[r.evidence_taxes_withheld,"taxesWithheld"],[r.evidence_refunds,"refunds"]].every(([coluna,chave])=>coluna===true||evidence[chave]===true); return { orderId:r.external_order_id,status:r.status,providerStatus:r.provider_status,occurredAt:new Date(r.occurred_at).toISOString(),closedAt:r.closed_at?new Date(r.closed_at).toISOString():null,currency:r.currency,gross:Number(r.gross),buyerShipping:r.buyer_shipping==null?null:Number(r.buyer_shipping),fulfillment:r.fulfillment,packageId:r.pack_id,financialStatus:!r.statement_settled?"pending":complete?"complete":"partial",items:r.items }; });
  const total = rows[0]?.total ?? 0;
  // Cards do monitor (E2 do Monitor Unificado, 28/08/2026): agregado do PERÍODO
  // inteiro, sem os filtros da tabela — os cards descrevem o período, a tabela
  // descreve o filtro. Mesma régua de receita do dashboard (status confirmados)
  // e a MESMA expressão coluna-primeiro de liquidação usada no resto do canal.
  const [resumo] = await dbQuery<{ confirmados: number; receita: string | null; conciliados: number; currency: string | null }>(
    `SELECT COUNT(*) FILTER (WHERE status IN ('paid','shipped','delivered'))::int AS confirmados,
            SUM(gross) FILTER (WHERE status IN ('paid','shipped','delivered')) AS receita,
            COUNT(*) FILTER (WHERE status IN ('paid','shipped','delivered')
              AND (financial_settled OR COALESCE((raw #>> '{_sellercore,statementSettled}')::boolean,false)))::int AS conciliados,
            MAX(currency) AS currency
       FROM workspace_channel_orders
      WHERE workspace_id=$1 AND provider=$2 AND connection_id=$3 AND occurred_at BETWEEN $4 AND $5`,
    [currentWorkspaceId(), PROVIDER, connection.id, period.from, period.to]
  );
  const confirmados = resumo?.confirmados ?? 0;
  const conciliados = resumo?.conciliados ?? 0;
  return {
    items,
    detail: orderId ? (items[0] ?? null) : undefined,
    page: pageMetadata(page, total, items.length),
    summary: {
      confirmados,
      // 0 confirmados => receita 0 é fato ("não vendeu"), não valor desconhecido.
      receita: Number(resumo?.receita ?? 0),
      conciliados,
      aguardandoExtrato: Math.max(0, confirmados - conciliados),
      currency: resumo?.currency ?? "BRL",
    },
