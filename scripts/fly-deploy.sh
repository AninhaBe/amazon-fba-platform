#!/bin/bash
# Deploy do NEXO no Fly.io — ver docs/adr/ADR-015.
#
# Por que existe (mesma razao do scripts/docker-build.sh):
# NEXT_PUBLIC_* sao substituidas DURANTE o `next build`, entao precisam entrar
# como --build-arg. `fly deploy` cru nao passa nada disso e a imagem sobe com as
# credenciais VAZIAS — o app funciona, o health check passa, e so a tela de login
# denuncia ("acesso bloqueado ate as credenciais do Supabase serem configuradas").
#
...
#
# Uso:  wsl bash scripts/fly-deploy.sh
set -euo pipefail
