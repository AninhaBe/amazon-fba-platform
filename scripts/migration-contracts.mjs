...
// v3 (ADR-012): removida a exigência de runtime role dedicada. O deployment conecta
// como dono do banco, então GRANT e policy `USING (true)` não isolavam nada. Ficam
// os REVOKE e a RLS ligada sem policy, que é o que protege de fato.
export const FINANCIAL_LEDGER_CONTRACT_VERSION = 3;
...
), actual_columns AS (SELECT col.table_name,col.column_name,col.data_type typ,(col.is_nullable='YES') nullable FROM information_schema.columns col WHERE col.table_schema=current_schema() AND col.table_name LIKE 'workspace_financial_%'),
...
SELECT ((SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname=current_schema() AND c.relname LIKE 'workspace_financial_%') + (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname=current_schema() AND p.proname IN ('financial_checkpoint_claim','financial_checkpoint_advance'))) object_count,
 ...
 NOT EXISTS(SELECT 1 FROM information_schema.table_privileges WHERE table_schema=current_schema() AND table_name LIKE 'workspace_financial_%' AND grantee<>current_user) acl_tables_closed,
 NOT EXISTS(SELECT 1 FROM information_schema.routine_privileges WHERE specific_schema=current_schema() AND routine_name IN ('financial_checkpoint_claim','financial_checkpoint_advance') AND grantee<>current_user) acl_functions_closed,
 ...
 NOT EXISTS(SELECT 1 FROM pg_policies WHERE schemaname=current_schema() AND tablename LIKE 'workspace_financial_%') no_policies;
`;
